...
public static class EventEndpoints
{
    public static void MapEventEndpoints(this WebApplication app)
    {
        ...
        group.MapGet("/range", async (
            ...
            IMediator mediator) =>
        {
            var query = new GetEventsByDateRangeQuery(startDate, endDate, instituteId, centreId);
            var result = await mediator.Send(query);
        ...
        group.MapPut("/{id:guid}", async (Guid id, [FromBody] UpdateEventCommand command, IMediator mediator) =>
        {
        ...
        group.MapPost("/{id:guid}/cancel", async (Guid id, [FromBody] CancelEventCommand command, IMediator mediator) =>
        {
            if (id != command.EventId)
                return Results.BadRequest("Event ID mismatch");
        ...
        group.MapDelete("/{id:guid}", async (Guid id, [FromQuery] Guid deletedBy, IMediator mediator) =>
        {
            var command = new DeleteEventCommand(id, deletedBy);
            await mediator.Send(command);
            return Results.NoContent();
        })
        ...
        group.MapPost("/{id:guid}/rsvp", async (Guid id, [FromBody] RsvpEventCommand command, IMediator mediator) =>
        {
            if (id != command.EventId)
                return Results.BadRequest("Event ID mismatch");
